...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const [hue, saturation, lightness] = primaryColor.split(' ').map(Number);

      // Adjust lightness for dark mode
      const isDarkMode = document.documentElement.classList.contains('dark');
      const adjustedLightness = isDarkMode ? Math.min(lightness + 20, 100) : lightness;
      ...
      dots.forEach(dot => {
        ...
        const gradient = ctx.createRadialGradient(dot.x, y, 0, dot.x, y, dot.r * 2);

        // Use theme colors with varying opacity
        const baseOpacity = isDarkMode ? 0.9 : 0.8;
        const fadeOpacity = isDarkMode ? 0.2 : 0.1;

        gradient.addColorStop(
          0,
          `hsla(${hue}, ${saturation}%, ${adjustedLightness}%, ${baseOpacity})`
        );
        gradient.addColorStop(
          1,
          `hsla(${hue}, ${saturation}%, ${adjustedLightness}%, ${fadeOpacity})`
        );

        ctx.fillStyle = gradient;
        ctx.shadowColor = `hsla(${hue}, ${saturation}%, ${adjustedLightness}%, ${isDarkMode ? 0.7 : 0.5})`;
        ctx.shadowBlur = isDarkMode ? 16 : 12;
        ctx.fill();
        ctx.shadowBlur = 0;
      });
  ...
  return (
    <section className="min-h-screen flex items-center justify-center relative pt-32 overflow-hidden">
      ...
      <motion.div
        ...
        style={{
          background:
            'radial-gradient(ellipse at 60% 40%, hsl(var(--background)) 60%, transparent 100%)',
        }}
        aria-hidden
      >
        <motion.svg
