...
export default function BlueprintRoute() {
  ...
  const [lastMarkerPosition, setLastMarkerPosition] = useState<{ x: number; y: number } | null>(
    null
  );
  ...
  useLayoutEffect(() => {
    function getMarkerPositions() {
      ...
      // After calculating marker positions
      if (points.length > 0) {
        const lastMarker = points[points.length - 1];
        setLastMarkerPosition(lastMarker);
      }
    }
    getMarkerPositions();
  ...
  return (
    <>
      ...
      <div className={styles.routeContainer}>
        ...
        >
          ...
          {markerPoints.map((pt, i) => {
            ...
            if (i === markerPoints.length - 1) {
              // Skip rendering the last marker here; it will be rendered on top below
              return null;
            }
            // Other markers
          ...
          {/* Render the last marker (plug) on top */}
          {markerPoints.length > 0 &&
            (() => {
              const i = markerPoints.length - 1;
        ...
        {/* Add electricity effect for the last marker */}
        {lastMarkerPosition && (
          <ElectricityEffect
