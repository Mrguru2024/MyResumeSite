...
// Minimal test button for debugging React event attachment
if (typeof window !== "undefined") {
  const testBtn = document.getElementById("test-sound-btn");
  if (!testBtn) {
    const btn = document.createElement("button");
    btn.id = "test-sound-btn";
...
function useSoundManager() {
  ...
  useEffect(() => {
    ...
    const stored = window.localStorage.getItem("blueprintroute-muted");
...
function SoundToggle({
  ...
  return (
    <button
      ...
      className="fixed top-4 left-4 z-[9999] bg-red-500 border-4 border-yellow-400 rounded-full p-4 shadow-2xl pointer-events-auto"
...
export default function BlueprintRoute() {
  ...
  return (
    <>
      ...
      <button
        style={{ position: 'fixed', top: 60, left: 0, zIndex: 99999, background: 'blue', color: 'white' }}
      ...
      <div className={styles.routeContainer}>
        ...
        {/* Sound toggle button OUTSIDE the routeContainer for guaranteed interactivity */}
